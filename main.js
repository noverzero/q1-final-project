...
//create an array of headliner names
      ...
      //write a function that takes the eventsArr and returns an array of objects with key = pick-up location name, value = quantity_sold


      // let ridersByLocationArr = []
      // let ridersByLocationObj = {}
      // function getRidersByLocation(){
      //
      //   for(let ii = 0; ii < eventsArr.length; ii++){
      //     let classes = eventsArr[ii].ticket_classes
      //     console.log("classes $$$$$$" , classes)
      //     for (let ii = 0; ii < classes.length; ii++){
      //      ridersByLocationObj[classes[ii].name.substring(indexof(), 15)] = classes[ii].quantity_sold
      //     console.log("ridersByLocationObj |||||||||||" , ridersByLocationObj)
      //     //console.log("ridersByLocation*****" , ridersByLocationObj)
      //     //ridersByLocationArr.push(ridersByLocationObj)
      //     }
      //   }
      // }
      // getRidersByLocation()
      ...
      //for each event get total number of riders from each location
      //create object of roundtrip miles for each location
      let miles = {
        boulderRrx: 54.2,
        focoRrx: 145.4,
        denverRrx: 43.2,
      ...
      //calculate bus fuel consumed:
      //number of buses used = number of riders / average bus capacity
        let busesUsed = Math.ceil(totalRidersArr[1] / bus.cap)
      console.log("busesUsed --->>" , busesUsed)
...
//Build the table!
  ...
  //retreive previous stored eventsArr from local storage
    ...
    console.log("storedEvents:o :o :o :o" , storedEvents)

    if(storedEvents){
      alert("yep!  storedEvents would work!")
      //create an array of headliner names
            let headlinersArr = []
            function getHeadliners(){
              for(let ii = 0; ii < storedEvents.length; ii++){
              headlinersArr.push(storedEvents[ii].name.text.substring((0), storedEvents[ii].name.text.indexOf("*")-1))
              }
              return headlinersArr
            }
            //getHeadliners()
            ...
            function getDates(){
              for(let ii = 0; ii < storedEvents.length; ii++){
              datesArr.push(storedEvents[ii].start.local.substring(0, 10))
              }
              return datesArr
            }
            //getDates()
            ...
            function getVenues(){
              for(let ii = 0; ii < storedEvents.length; ii++){
                venuesArr.push(storedEvents[ii].name.text.substring((storedEvents[ii].name.text.lastIndexOf("*")+5), storedEvents[ii].name.text.lastIndexOf("(")-1))
              }
              return venuesArr
            }
            //getVenues()
            ...
            //Write a function that takes the storedEvents, and returns an Array with the sum of riders from all locations as an index an array.
            //iterate through the storedEvents
            //iterate through each pick up location.
            //get the quantity and add it to the eventTotal accumulator variable for that event
            //push the total to the totalRidersArr
            //
            ...
            function getRidersTotal(){
              for(let ii = 0; ii < storedEvents.length; ii++){
                let ticketClasses = storedEvents[ii].ticket_classes
                let eventTotal = 0
                for(let ii = 0; ii < ticketClasses.length; ii++){
                  eventTotal += ticketClasses[ii].quantity_sold
                }
            ...
            //write a function that takes the storedEvents and returns an array of objects with key = pick-up location name, value = quantity_sold
            ...
            //
            //   for(let ii = 0; ii < storedEvents.length; ii++){
            //     let classes = storedEvents[ii].ticket_classes
            //     console.log("classes $$$$$$" , classes)
            //     for (let ii = 0; ii < classes.length; ii++){
            //      ridersByLocationObj[classes[ii].name.substring(indexof(), 15)] = classes[ii].quantity_sold
            //     console.log("ridersByLocationObj |||||||||||" , ridersByLocationObj)
